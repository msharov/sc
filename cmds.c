...
int get_rcqual (int ch)
{
	...
	case ctl('n'):
	case ctl('p'):	if (ch == 'd')
			    sprintf(line,"deleterow [range] ");
			else if (ch == 'y')
			    sprintf(line,"yankrow [range] ");
			else if (ch == 'Z')
	...
	case 'H':
	case 'L':	if (ch == 'd')
			    sprintf(line,"deletecol [range] ");
			else if (ch == 'y')
			    sprintf(line,"yankcol [range] ");
			else if (ch == 'Z')
			    sprintf(line,"hide [range] ");
			else
			    return (0);
...
void formatcol (int arg)
{
	...
	if (c >= '0' && c <= '9') {
		...
		case ' ':
		    if (arg == 1)
			sprintf(line, "format [for column] %s ", coltoa(curcol));
		    else {
			sprintf(line, "format [for columns] %s:", coltoa(curcol));
			sprintf(line+strlen(line), "%s ", coltoa(curcol+arg-1));
		    }
		    linelim = strlen(line);
		    insert_mode();
		...
		case '=':
		    ...
		    if ((c = nmgetch()) >= '0' && c <= '9') {
			if (colformat[c-'0']) {
			    sprintf(line, "format %c = \"%s\"", c, colformat[c-'0']);
			    edit_mode();
			    linelim = strlen(line) - 1;
			} else {
			    sprintf(line, "format %c = \"", c);
			    insert_mode();
			    linelim = strlen(line);
			}
			error(" ");
...
void printfile(char *fname, int r0, int c0, int rn, int cn)
{
    ...
    if (!pline && (pline = scxmalloc((unsigned)(FBUFLEN * ++fbufs_allocated))) == NULL) {
	...
	for (pp = ATBL(tbl, row, col=c0); col<=cn;
		...
		while (c > (int)(fbufs_allocated * FBUFLEN)) {
		...
		if ((*pp)->flags&is_valid) {
			...
			if ((pline = ((char *)scxrealloc ((char *)pline, (unsigned)(FBUFLEN * ++fbufs_allocated)))) == NULL) {
			    error("Realloc failed in printfile()");
			    return;
			}
		    }
		    if ((*pp)->cellerror)
			sprintf(pline+plinelim, "%*s", fwidth[col], ((*pp)->cellerror == CELLERROR ? "ERROR " : "INVALID "));
		    else {
			char *cfmt;
			...
			if (cfmt) {
			    ...
			    char field[FBUFLEN];
			    engformat(realfmt[col], fwidth[col], precision[col], (*pp) -> v, field, sizeof(field));
			    sprintf(pline+plinelim, "%*s", fwidth[col], field);
			}
		    }
		...
		if ((s = (*pp)->label)) {
		    ...
		    // A string started with backslash is defining repetition char
		    slen = strlen(s);
		    if (*s == '\\' && *(s+1) != '\0')
			slen = fwidth[col];
		    while (slen > fieldlen && nextcol <= cn && !((nc = lookat(row,nextcol))->flags & is_valid) && !(nc->label)) {
		    ...
		    while (c + fieldlen + 2 > (int)(fbufs_allocated * FBUFLEN)) {
			if ((pline = ((char *)scxrealloc ((char *)pline, (unsigned)(FBUFLEN * ++fbufs_allocated)))) == NULL) {
...
void tblprintfile (char *fname, int r0, int c0, int rn, int cn)
{
	...
	for (pp = ATBL(tbl, row, col=c0); col<=cn; col++, pp++) {
		...
		if ((*pp)->flags&is_valid) {
		    if ((*pp)->cellerror)
			fprintf (f, "%*s", fwidth[col], ((*pp)->cellerror == CELLERROR ? "ERROR" : "INVALID"));
		    else if ((*pp)->format) {
		        char field[FBUFLEN];
			if (*((*pp)->format) == ctl('d')) {
			    time_t v = (time_t) ((*pp)->v);
...
static void syncref (struct enode *e)
{
	...
	switch (e->op) {
	    case 'v':
		if (e->e.v.vp->flags & is_cell_cleared) {
		    e->op = ERR_;
		    e->e.o.left = NULL;
		    e->e.o.right = NULL;
...
void forwcol (int arg)
{
	...
	if (curcol < maxcols - 1)
	    curcol++;
	else if (!growtbl(GROWCOL, 0, arg))	// get as much as needed
	    break;
	else
	    curcol++;
...
void forwrow (int arg)
{
	...
	if (currow < maxrows - 1)
	    currow++;
	else if (!growtbl(GROWROW, arg, 0))	// get as much as needed
	    break;
	else
	    currow++;
    ...
    int flcols, int frcols)	// width of left and right sides of frame
{
    ...
    // This figures out if the label is allowed to slop over into the next blank field
